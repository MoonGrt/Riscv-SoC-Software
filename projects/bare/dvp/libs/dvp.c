...
{
    DVPx->VP_CR = ((VP_InitStruct->Mode & 0x7) << 1) |           // 1位使能 + 2位模式 [2:1]
...
{
    // 配置 VI_CR
    DVPx->VI_CR = ((DVP_InitStruct->VIMode & 0x3) << 1); // 1位使能 + 2位模式 [2:1]
    // 配置 VP_CR
    VP_Init(DVPx, &DVP_InitStruct->VPMode);
    // 配置 VO_CR
    DVPx->VO_CR = ((DVP_InitStruct->VOMode & 0x3) << 1); // 1位使能 + 2位模式 [2:1]
}
