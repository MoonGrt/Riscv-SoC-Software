...
void init_filesystem(void) {
    ...
    // 创建根目录中的文件
    rt_strncpy(fs.root.files[fs.root.file_count].name, "README.txt", MAX_NAME_LENGTH);
    rt_strncpy(fs.root.files[fs.root.file_count].content, "File system!", MAX_CONTENT_LENGTH);
    fs.root.file_count++;
...
//             printf("File %s content:\n%s\r\n", file_name, fs.current_dir->files[i].content);
//             return;
//         }
//     }
